...
def get_args():
    ...
    parser.add_argument("--count", type=int, help="dataset items to use", default=500)
    ...
    parser.add_argument("--samples-per-query", type=int,
                        help="mlperf multi-stream sample per query", default=1)
...
def main():
    ...
    for i in range(0, count, query_size):
        img, _ = ds.get_samples(list(range(i, min(i+query_size, count))))
        img_pickle = pickle.dumps(img)
        s = time.time()
        zimg = gzip.compress(img_pickle, 0)
        e = time.time()
        compression_rate.append(len(zimg)/len(img_pickle))
        compression_time.append(e-s)
    ...
    cli_colors.color_print(f"Average compression rate: {(sum(compression_rate)/len(compression_rate)):2f}", cli_colors.CYAN)
    cli_colors.color_print(f"Average compression time: {(sum(compression_time)/len(compression_time)):2f}", cli_colors.CYAN)
