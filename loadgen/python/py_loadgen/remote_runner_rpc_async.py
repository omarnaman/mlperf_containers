...
class RemoteRunnerBase:
    ...
    def connect(self):
        self.channel = grpc.aio.insecure_channel('localhost:8086')
    ...
    async def predict_remote(self, qitem: Item):
        item_pickle = pickle.dumps(qitem.img)
        request = basic_pb2.RequestItem(items=item_pickle)
        response: basic_pb2.ItemResult = await self.stub.InferenceItem(request)
    ...
    def run_one_item(self, qitem):
        ...
        try:
            ...
            cli_colors.color_print("Predict Returned", cli_colors.BLUE_SHADE1)
